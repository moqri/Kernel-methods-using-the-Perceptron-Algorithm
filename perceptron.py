...
def predict(state, kernel, x_i):
    ...
    Args:
        state: The state returned from updated_state
    ...
    #print("xs has ", len(xs), "elements")
    #print("betas has ", len(betas), "elements")

    alt1 = False
    if alt1:
        for j in range(len(betas)):
            var += (betas[j] * kernel(xs[j], x_i))
        return sign(var)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    if alt1:
        if len(xs) == 1:
            betas[0] = (betas[0] + alpha * (y_i - predict(state, kernel, x_i)))
        else:
            betas.append(betas[-1] + alpha*(y_i - predict(state, kernel, x_i)))

        #print("after the update, betas has ", len(betas), "elements")
    ...
    alt3 = True
    if alt3:
        betas[-1] = betas[-1] + alpha * (y_i - predict(state, kernel, x_i))
